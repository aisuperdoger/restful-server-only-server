...
void Controller::initialize(const string& url, HttpServer* server) {
    mapping_url_ = url;  // 形如"/api"

    for (auto& router : router_mapping_.routers) {
        for (auto& method : router.second) {
            server->add_router(
                iLogger::format("%s%s", mapping_url_.c_str(), router.first.c_str()),       // 获取形如"/api/getReturn"的字符串， router.first.c_str()为字符串getReturn
                bind(&Controller::process_module, this, placeholders::_1, method.second),  // method.second为main.cpp中定义的函数getReturn   method.second为http的方法如GET、POST
                method.first);                                                             // add_router函数向router_map_中添加url和处理函数的映射，router_map_[url_remove_back][method] = callback;
                                                                                           // 使用bind使得process_module传入不同的函数，实现不同的功能
        }
    }
}
...
void Controller::process_module(const shared_ptr<Session>& session, const ControllerProcess& func) {
    ...
    if (!(session->request.headers.find("Content-Type") == session->request.headers.end())  // 代表头部信息中是否有Content-Type字段
        ...
        auto param = Json::parse_string(session->request.body);  // 发送过来的格式要求是json格式，但是发送过来的是字符串，需要将字符串转换为Json。
    }
    ...
    if (session->response.write_mode == ResponseWriteMode_WriteReturnJson) {
        session->response.output.writeData(ret.toStyledString());  // 将返回的内容存储在memoryWrite_中
    }
...
void HttpServerImpl::add_router(const string& url, const HandlerCallback& callback, const string& method) {
    string url_remove_back(url);  // 进行一次字符串复制，我还以为url_remove_back是个函数
    if (!url_remove_back.empty() && url_remove_back.back() != '/' || url_remove_back.empty())
        url_remove_back.push_back('/');  // 保证url以"/"结尾
