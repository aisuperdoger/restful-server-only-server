...
#define DefRequestMapping(name)                                                                                          \
    ...
    Json::Value name(const Json::Value& param)  // __##name##_register__是什么东西？答：这里就是定义了名字为__##name##_register__的函数。 ##会把两个语言符号组合成单个语言符号。 前面没有#的name代表：函数名为name，而不是字符串name
...
class Controller {
   protected:
    typedef std::function<Json::Value(const Json::Value&)> ControllerProcess;  // ControllerProcess用于指向形参为Value且返回值为Value的函数
    struct RequestMapping {
        std::unordered_map<std::string, std::unordered_map<std::string, ControllerProcess>> routers;  // 为什么使用unordered_map？答：因为unordered_map的查找速度更快,map和multimap使用红黑树实现，unordered_map使用哈希表实现
    } router_mapping_;                                                                                // 定义链接和处理函数的对应关系 // 为什么不直接使用routers，而使用router_mapping_？

    std::string mapping_url_;
    std::unordered_map<std::thread::id, std::shared_ptr<Session>> current_session_;  // 每个客户与服务器进行会话都需要用一个线程去处理？？，不用epoll吗？
    std::mutex session_lock_;

   public:
    void initialize(const std::string& url, HttpServer* server);
    virtual void process(const std::shared_ptr<Session>& session);  // 控制器中做了发送数据的事情？？
