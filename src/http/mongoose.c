...
void mg_mgr_init(struct mg_mgr *m, void *user_data) {
  ...
  memset(&opts, 0, sizeof(opts)); // 将结构体opts中的字节都置为0
...
struct mg_connection *mg_bind(struct mg_mgr *srv, const char *address,
                              MG_CB(mg_event_handler_t event_handler,
                                    void *user_data)) {  // MG_CB只返回callback，那传入user_data的意义在哪里
...
void mg_http_serve_file_internal3(struct mg_connection *nc,
                                 struct mg_str req_range,
                                 struct mg_str req_connection,
                                 struct mg_str req_proto,
                                 const time_t last_modify,
                                 const char *path,
                                 struct mg_str mime_type,
                                 struct mg_str encoding,
                                 struct mg_str extra_headers) {
  ...
  if (mg_stat(path, &st) != 0 || (pd->file.fp = mg_fopen(path, "rb")) == NULL) {
    ...
    time_t t = (time_t) mg_time();
    int64_t r1 = 0, r2 = 0, cl = st.st_size;
    struct mg_str *range_hdr = req_range.len == 0 ? NULL : &req_range;
    int n, status_code = 200;
...
#if !MG_DISABLE_HTTP_KEEP_ALIVE
    {
      struct mg_str *conn_hdr = req_connection.len == 0 ? NULL : &req_connection; //mg_get_http_header(hm, "Connection");
      if (conn_hdr != NULL) {
        pd->file.keepalive = (mg_vcasecmp(conn_hdr, "keep-alive") == 0);
      } else {
        pd->file.keepalive = (mg_vcmp(&req_proto, "HTTP/1.1") == 0);
      }
    }
#endif

    st.st_mtime = last_modify;
    mg_http_construct_etag(etag, sizeof(etag), &st);
    mg_gmt_time_string(current_time, sizeof(current_time), &t);
    mg_gmt_time_string(last_modified, sizeof(last_modified), &st.st_mtime);
    mg_send_response_line_s(nc, status_code, extra_headers);
...
void mg_http_serve_file3(struct mg_connection *nc,
                        struct mg_str req_range,
                        struct mg_str req_connection,
                        struct mg_str req_proto,
                        const time_t last_modify,
                        const char *path, const struct mg_str mime_type,
                        const struct mg_str extra_headers) {
  mg_http_serve_file_internal3(nc,
      req_range, req_connection, req_proto, last_modify, path, mime_type, mg_mk_str(NULL), extra_headers);
}
